...
fn to_snake_case(s: &str) -> String {
    ...
    let final_str: String = trimmed.chars().filter(|c| c.is_alphanumeric() || *c == '_').collect();
...
fn escape_rust_keyword(s: &str) -> String {
    match s {
        ...
        | "where" | "while" | "async" | "await" | "dyn" => format!("r#{}", s),
        _ => s.to_string(),
    }
}
    ...
    match value {
        ...
        serde_json::Value::Array(arr) => {
            if arr.is_empty() {
                ...
                let elem_type = generate_rust_type(&arr[0], field_name, structs);
        ...
        serde_json::Value::Object(obj) => {
            ...
            for key in keys {
                let val = &obj[key];
                let mut rust_field_name = to_snake_case(key);
                let mut serde_attr = None;
                
                if rust_field_name != *key {
                    serde_attr = Some(format!("#[serde(rename = \"{}\")]", key));
                }
                ...
                let rust_type = generate_rust_type(val, &sub_field_name, structs);
...
#[component]
pub fn JsonToRust() -> impl IntoView {
    ...
    let conversion_result = Memo::new(move |_| {
        ...
        let root_type = generate_rust_type(&parsed, &name, &mut structs);
        ...
        for s in structs.iter().rev() {
            output.push_str("#[derive(Default, Debug, Clone, PartialEq, Serialize, Deserialize)]\n");
  ...
  "manufacturer": {
    ...
    view! {
        <div class="space-y-6">
            // Header
            <div>
                <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">"JSON to Rust Struct"</h1>
                <p class="mt-1 text-slate-500 dark:text-slate-400 text-sm">"Convert JSON objects into fully typed, nested Rust struct definitions with serde derives."</p>
            </div>
